...
import { assignDefaultRole } from '../utils/initializeUserRoles';
...
export const useAuth = () => {
  ...
  const updateUser = async (supabaseUser: SupabaseUser | null) => {
    ...
    // Obtener el rol del usuario desde user_roles (ahora que las políticas RLS están arregladas)
    ...
    try {      
      ...
      if (error) {
      // Fallback: usar rol basado en email
        userRole = getRoleFromEmail(supabaseUser.email || '');
      } else if (roleData && roleData.role) {
        userRole = roleData.role as UserRole;
      } else {
        // No hay rol en la base de datos, usar fallback por email
        userRole = getRoleFromEmail(supabaseUser.email || '');        
        // Intentar insertar el rol en la base de datos para futuras consultas
        try {
          await supabase
            .from('user_roles')
